# Brutal Prism Design System

## Overview

Brutal Prism is a bold, tech-focused design system that combines the vibrant tech color palette of Nexus Prism with the geometric, hard-edged aesthetic of Neo Brutalist design. It features thick borders, offset shadows, and a dark tech-inspired color scheme, creating a unique visual identity that's both modern and impactful. The system uses Bootstrap 5.3.3 only for structure and responsive layout, with all visual styles completely customized.

## Design Principles

1. **Brutalist Geometry** : Sharp edges, no border-radius, thick borders
2. **Tech Color Palette** : Indigo, purple, cyan on dark backgrounds
...
### Neutral Colors - Dark Tech Backgrounds
...
--color-bg-light: #F8FAFC;       /* Slate 50 - light background (for contrast) */
...
--color-border-glow: rgba(99, 102, 241, 0.5); /* Glowing border effect (optional) */
```
...
All components use custom visual styles with a unique brutalist-tech identity.
...
- Dark tech backgrounds (slate 800/900)
...
- Tech colors on dark backgrounds maintain excellent contrast
...
- **Dark Background + Tech Colors** : Maximum impact and readability
- **Light Background (sparingly) + Dark Text** : For contrast sections

### Usage Guidelines

- Use dark backgrounds (slate 900/800) as primary surfaces
